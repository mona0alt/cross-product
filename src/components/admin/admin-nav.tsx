...
export function AdminNav({ items }: { items: AdminNavItem[] }) {
  ...
  return (
    <aside className="flex h-full flex-col text-slate-700">
      <div className="flex h-14 items-center justify-center border-b border-r border-white/10 bg-[#07111f] px-4">
        <Image src={adminLogoSrc} alt="FBGM" width={563} height={670} className="h-11 w-auto" unoptimized />
      </div>

      <nav className="flex-1 border-r border-admin-border bg-white pt-4">
        {items.map((item) => {
