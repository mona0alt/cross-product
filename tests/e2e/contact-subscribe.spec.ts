...
test('contact and subscribe forms submit successfully', async ({ page }) => {
  ...
  await expect(page.getByText('Your message has been submitted. Our team will follow up shortly.')).toBeVisible();

  await page.goto('/en/subscribe');
  await page.getByPlaceholder('Email').fill('buyer@example.com');
  await page.getByRole('button', { name: 'Subscribe' }).click();
  await expect(page.getByText('You are subscribed. We will send the latest product updates.')).toBeVisible();
});
